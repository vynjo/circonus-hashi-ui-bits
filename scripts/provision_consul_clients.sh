...
create_cgc -query "consul*consul*http*GET*v1*health*state*_" -title "Consul http GET v1 health state" -tags "creator:api,role:client,service:consul,data-type:histogram,group:primary"
