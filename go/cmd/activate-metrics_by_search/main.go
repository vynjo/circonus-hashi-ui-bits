...
func findMetrics(id string) ([]metricSearchResult, error) {
	...
	metricSearchURL := fmt.Sprintf("/metric?search=(active:0)*%s*", id)
...
func activateMetrics(checkBundleID string, metricList checkBundleMetricList) error {
	q := url.Values{}
	q.Set("search", "(active:0)*%s*")
	apiURL := url.URL{
...
func main() {
	...
	if len(foundMetrics) > 0 {
		if err = activateMetrics(checkBundleID, metricList); err != nil {
			log.Printf("Error: %+v\n", err)
		}
	}
}
