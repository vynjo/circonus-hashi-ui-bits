...
func getLostAllocations() ([]Allocation, error) {
	allocations, err := getAllocations()
	if err != nil {
		return nil, err
	}
	...
	for _, allocation := range allocations {
		if allocation.ClientStatus == "lost" {
			completed = append(completed, allocation)
		}
	}
