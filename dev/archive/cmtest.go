...
func main() {
	...
	currentallocs, err := http.Get("http://104.196.144.155:4646/v1/allocations");
	if err != nil {
		panic(err)
	}
	//Wait for the full response
	defer currentallocs.Body.Close()
	// Read the entire body
	hashibody, err := ioutil.ReadAll(currentallocs.Body)
	if err != nil {
		panic(err)
	}	
// 	log.Printf("Start")
	// Turn Hashi API call into usable structure (allocs, and complete_allocs. The later is used to mark Circonus
	//		metrics as available
	allocs := make([]Allocation, 0)
	complete_allocs := make([]Allocation, 0)
// 	running_allocs := make([]Allocation, 0)
	json.Unmarshal(hashibody, &allocs)
	fmt.Println("There are", len(allocs), "allocations")
	// Find the Completed Allocations
	for n := range allocs {
// 		fmt.Println("Name: ", allocs[n].Name, "on Node:", allocs[n].NodeID, "ID:", allocs[n].ID)
		if (allocs[n].ClientStatus == "complete") {
			fmt.Println("Name: ", allocs[n].Name, "on Node:", allocs[n].NodeID, "ID:", allocs[n].ID)
			fmt.Println(allocs[n])
			fmt.Println(allocs[n].DesiredStatus)
			complete_allocs = append(complete_allocs, allocs[n])
		}
	}
	// Now find Associated Circonus Metrics
